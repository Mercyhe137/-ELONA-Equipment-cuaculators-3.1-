...
void dizi(vector<int> initVals, int maxLimit, int &maxVal,int &currentVal, int &currentLimit,vector<int> &usedVals, vector<int> &halfVals)
{
    int n = initVals.size();
    int minCount = n + 1;
    for(int i = 0; i < n; i++){
        if(currentVal + floor(initVals[i] / 2) == currentLimit){
            usedVals.clear();
            halfVals.clear();
            usedVals.push_back(initVals[i]);
            minCount = 1;
            maxVal = maxLimit;
            break;
        }
...
ifstream inputFile(fileName);
    ...
    for (char ch : str3) {
        ...
        cout<<"消耗能力卷轴数：" <<(usedVals.size()) << endl;
        cout << endl;
        cout << "是否采用并保存该方案？（Y/N）";
        char decide;
        cin >> decide;
   ...
   newString += "\n消耗能力卷轴数：#"+std::to_string( usedVals.size()) + "\n\n";
...
ifstream inputFile(fileName);
    ...
    cout << (fileName) << endl;
        ...
        int currentLimit;
        int bestVal;
        std::string str8 = "输入用作底子的装备的当前数值: ";
    for (char ch : str8) {
        std::cout << ch << std::flush;
        std::this_thread::sleep_for(std::chrono::milliseconds(20));
    }
       ...
       // cout<<maxLimit<<endl;
        ...
        vector<int> usedVals;
        vector<int> halfVals;
        dizi(initVals, maxLimit,  maxVal, currentVal, currentLimit,usedVals, halfVals);
